...
- **(2026-06-25) SMS — TFN NOT yet registered (corrected).** Confirmed in Brevo (Transactional → SMS): the US toll-free number has NOT been submitted — paying for SMS credits earlier did not register a number. To start: complete Brevo's toll-free registration **form** (banner in Transactional → SMS), then carrier review ~2–4 wks. App side is fully built and dormant: `notify-sweep`, heir/assignee/invite/report SMS, inbound-sms webhook, and `estate_users.sms_consent` capture. **ACTIVATION when approved: set Netlify env `BREVO_SMS_SENDER` to the approved toll-free number** (+ wire the inbound-SMS webhook with `INBOUND_SMS_SECRET`) → everything flips on; then verify a send.
