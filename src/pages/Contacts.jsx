  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl mx-auto w-full">
      ...
      <input
        ...
        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-200 mb-4"
      />
      ...
      <div className="space-y-4">
        {Object.entries(CONTACT_ROLES).map(([roleKey, roleLabel]) => {
          ...
          return (
            <div key={roleKey} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              <div className="px-4 py-2.5 bg-gray-50 border-b border-gray-100">
                <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">{roleLabel}</span>
              </div>
              <div className="divide-y divide-gray-100">
                {group.map(c => (
                  <Link key={c.id} to={`/contacts/${c.id}`} className="flex items-start gap-3 px-4 py-3 hover:bg-gray-50">
